/** Release-webview playback benchmark, using only an isolated synthetic project.
 * Build: npm run tauri -- build --features webdriver --no-bundle
 * Run: node tools/webdriver/playback.mjs [seconds=60] [steps=24] [rates=8,12,24,30] [--report-only]
 */
...
const reportOnly = process.argv[5] === "--report-only";
...
try {
  ...
  const environment = await evaluate(`done({userAgent:navigator.userAgent,viewport:[innerWidth,innerHeight],pixelRatio:devicePixelRatio})`);
  ...
  console.log(JSON.stringify({root, steps, preparationMs, environment}));
  const results = [];
  for (const rate of rates) {
    ...
    for (let elapsed = 0; elapsed < seconds; elapsed += 10) {
      ...
      console.log(JSON.stringify({rate, seconds:Math.min(seconds, elapsed + 10)}));
    }
    ...
    const measured = { rate, ...summary, sequential, frames: frames.length };
    results.push(measured);
    console.log(JSON.stringify(measured));
  }
  const path = join(root, "results.json");
  await writeFile(path, JSON.stringify({ steps, preparationMs, environment, results }, null, 2));
  console.log(`Results: ${path}`);
  if (!reportOnly && results.some((result) => Math.abs(result.displayedFps / result.rate - 1) > 0.05 || !result.sequential)) process.exitCode = 1;
} finally {
