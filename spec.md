...
**The recent list can be cleared.** A **Clear** beside the start screen's
*Recent* heading forgets every entry at once, shown only when there is
something to forget. It is asked for first, like the macro library's *delete
all* (§8.7): the list cannot be rebuilt except by opening the projects again.
It forgets shortcuts and nothing else — no project file is touched, and
*Recovered work* is a separate list with its own per-entry Discard. The clear
reaches the settings file before it is reported done, so a refused write says
so rather than emptying the screen and letting the list return at the next
launch.
...
- **Speed** as a colour ramp raster, sampled from the render tiles (§7.7), with
  ...
  **The gradients are chosen from a picker that shows them.** It is a
  ...
  own rather than the platform's, because a native menu renders nothing but
  text in its options and the one thing worth knowing about a gradient is
  what it looks like. Closed, it shows the run of colours in use; open, it
  shows them all. A gradient the build does not have is listed, since that is
  what the file says, and cannot be picked, since there is nothing to pick.
...
| `radial` | f32 % (−400…400) | diverge / converge — a **centred slider** (M29), *Divergence* at the left end and *Convergence* at the right, 0 — neither — in the middle and the default; the stored sign is untouched, positive diverging, the slider simply reversed. **It bends the flow and does not drive it** (M54): the outward component is added and the result taken back to the speed it came in at, so the amount is a bearing control and nothing else. Left as the plain sum, the new speed was the hypotenuse of the old one and the radial part — so *both* ends of the slider sped the field up, a convergence included, which is the opposite of what the word says. A full convergence against an exactly outward flow still cancels it to nothing, since scaling a vector of no length back up would be inventing a direction for it |
...
the adapter is software-only (use the reference CPU evaluator directly);
