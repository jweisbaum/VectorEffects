/** GPU resources for arbitrary map projections. Geographic source tiles remain
 * ordinary textures; only their screen mesh changes when the camera moves. */
import { project, projectedMesh, tileBounds, visibleBounds, type Camera, type Viewport, type VisibleTile } from "../camera";
import type { ImageDraw } from "../renderer";
import { clipPolygon, type GeographicMesh, type MeshVertex } from "./mesh";
...
out vec2 vUV;
void main(){vUV=aUV;gl_Position=vec4(aScreen.x/uViewport.x*2.0-1.0,1.0-aScreen.y/uViewport.y*2.0,0.0,1.0);}`;
...
export class ProjectedSurface {
  ...
  private mesh:GeographicMesh|null=null;
  ...
  constructor(private readonly gl:WebGL2RenderingContext){
    ...
    this.sampler=gl.getUniformLocation(this.program,"uTexture");this.framebuffer=gl.createFramebuffer()!;
  ...
  bindImage(camera:Camera,view:Viewport,image:ImageDraw):number {
    ...
    const gl=this.gl;gl.bindVertexArray(this.imageMesh.vao);gl.bindBuffer(gl.ARRAY_BUFFER,this.imageMesh.buffer);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(data),gl.DYNAMIC_DRAW);
    return data.length/4;
  }
  /** Screen-space graticule segments, adaptively curved and horizon-clipped. */
  bindGraticule(camera:Camera,view:Viewport,step:number):number {
    const key=JSON.stringify([camera,view,step]);
    if(key===this.lineKey){this.gl.bindVertexArray(this.lines.vao);return this.lines.count;}
    this.lineKey=key;
    const data:number[]=[],bounds=visibleBounds(camera,view),limit=Math.hypot(view.width,view.height)*2;
    const point=(lon:number,lat:number)=>project(camera,view,{lon,lat});
    function segment(lon0:number,lat0:number,lon1:number,lat1:number,depth=0):void {
      ...
      if(!av&&!bv&&!mv)return;
      if(av&&bv&&mv && ([a,b,m].every(p=>p.x < -8) || [a,b,m].every(p=>p.x > view.width+8)
        || [a,b,m].every(p=>p.y < -8) || [a,b,m].every(p=>p.y > view.height+8)))return;
