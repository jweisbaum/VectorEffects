...
The imported-raster follow-up found that both panels requested `RasterSequence::fastest_mps` on every playhead change, rescanning every sample of every imported frame on the UI thread. The ceiling is now computed when the sequence is constructed and reused by both GRIB and Zarr layers. A 24-frame 0.25° metadata regression check took at most 13 µs per request for either layer type, including the first request.

Offline fixtures are generated with `cargo run -p ve-app --example playback_rasters -- /tmp/ve-raster-fixtures 24`. Pass `--project=/tmp/ve-raster-fixtures/grib.veproj` or `--project=/tmp/ve-raster-fixtures/zarr.veproj` to the playback benchmark. The history fixture uses the same local GRIB backing file as a completed Zarr import; no archive is contacted. At a 1440 × 872 viewport and pixel ratio 2, GRIB improved from about 2.7 FPS in a 10-second baseline at the 30 FPS setting to about 30 FPS over a full minute. One-minute 24 FPS runs delivered about 23.9 FPS for both formats. These prepared runs were sequential, with no buffering or tile transfers. The Zarr 30 FPS measurement was interrupted by background-window throttling; a repeat reported a hidden, unfocused window throughout and is inconclusive. The benchmark now records window visibility and checks frame count against the entire elapsed time so a long trailing suspension cannot pass on the first-to-last-frame rate alone.
